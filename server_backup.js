...
app.post("/sendRescheduleAppointmentConfirmation", async (req, res) => {
  console.log("📩 Received request to send reschedule confirmation:", req.body);
  ...
  try {
    ...
    res.send({ message: "Reschedule confirmation emails sent successfully!" });
    ...
    try {
      ...
      const reminderRef = await db
        .collection("ReminderEmails")
        .add(reminderData);
      console.log("🗓️ Reminder updated in Firestore with ID:", reminderRef.id);
    } catch (firestoreError) {
      console.error("❌ Failed to save reminder in Firestore:", firestoreError);
    }
  } catch (error) {
