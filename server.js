...
app.post("/sendDonationThankYou", async (req, res) => {
  ...
  const receiptNumber = "sakhj312";
  const filename = `invoice-${transactionID || Date.now()}.pdf`;

  try {
    // Generate HTML from your existing template
    const invoiceHtml = donationThankYouTemplate(
      name,
      donationAmount,
      transactionID,
      isRecurring,
    ...
    const invoicePath = await generateInvoiceFromHTML(
      invoiceHtml,
      transactionID
    );
    ...
    const mailOptions = {
      ...
      html: invoiceHtml, // same as your email template
      attachments: [
        {
